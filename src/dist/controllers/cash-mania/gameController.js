...
const cashmanianotcash = __importDefault(require("../../jsons/cash-mania/notcash"));
const cashmaniacontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield cashmaniacontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield cashmanianotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * cashmaniacontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "cash-mania";
                const game_code = 1682240;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield cashmanianotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield cashmanianotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield cashmaniacontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = cashmaniacontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = cashmaniacontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield cashmanianotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    const ganhojson = cashmaniacontrol_logic.default.taskWinJsonData(resultadospin.json)
                    const jsonData = cashmaniacontrol_logic.default.GetWinJson(ganhojson, 0, ml, cs, bet, saldoatual);
                ...
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "cash-mania") {//大奖
                    const cartajson = cashmaniacontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = cashmaniacontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = cashmaniacontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, cashmaniacontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
