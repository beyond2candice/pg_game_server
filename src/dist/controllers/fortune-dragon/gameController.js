...
const fortunedragonnotcash = __importDefault(require("../../jsons/fortune-dragon/notcash"));
const fortunedragoncontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield fortunedragoncontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield fortunedragonnotcash.default.notcash(0, cs, ml));
                    return false;
				}
               ...
               // console.log(bet + "|" + fb);
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield fortunedragonnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield fortunedragonnotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                ...
                if (resultadospin.result === constans.LOST) {//输
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield fortunedragonnotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield fortunedragonfunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                else if (resultadospin.result == constans.WIN) {
                    const ganhojson = fortunedragoncontrol_logic.default.taskWinJsonData(resultadospin.json)
                    const jsonData = fortunedragoncontrol_logic.default.GetWinJson(ganhojson, 0, ml, cs, bet, saldoatual);
                ...
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "fortune-dragon") {//大奖
                    const cartajson = fortunedragoncontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = fortunedragoncontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = fortunedragoncontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, fortunedragoncontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield fortunedragonfunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield fortunedragonfunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
