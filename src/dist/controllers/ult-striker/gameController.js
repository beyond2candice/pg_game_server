...
const ultstrikerfunctions_1 = __importDefault(require("../../functions/ult-striker/functions"));
...
const ultstrikernotcash = __importDefault(require("../../jsons/ult-striker/notcash"));
const ultstrikercontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield ultstrikercontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield ultstrikernotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * ultstrikercontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "ult-striker";
                const game_code = 1489936;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield ultstrikernotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield ultstrikernotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield ultstrikercontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = ultstrikercontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = ultstrikercontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield ultstrikernotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield ultstrikerfunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "ult-striker") {//大奖
                    const cartajson = ultstrikercontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = ultstrikercontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = ultstrikercontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, ultstrikercontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield ultstrikerfunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield ultstrikerfunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
