...
const dinerdelightsfunctions_1 = __importDefault(require("../../functions/diner-delights/functions"));
...
const dinerdelightsnotcash = __importDefault(require("../../jsons/diner-delights/notcash"));
const dinerdelightscontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield dinerdelightscontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield dinerdelightsnotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * dinerdelightscontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "diner-delights";
                const game_code = 1372643;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield dinerdelightsnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield dinerdelightsnotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield dinerdelightscontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = dinerdelightscontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = dinerdelightscontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    const cartajson = dinerdelightscontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = dinerdelightscontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = dinerdelightscontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, dinerdelightscontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
