...
const wingsiguazufunctions_1 = __importDefault(require("../../functions/wings-iguazu/functions"));
...
const wingsiguazunotcash = __importDefault(require("../../jsons/wings-iguazu/notcash"));
const wingsiguazucontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield wingsiguazucontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield wingsiguazunotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * wingsiguazucontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "wings-iguazu";
                const game_code = 1747549;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield wingsiguazunotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield wingsiguazunotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield wingsiguazucontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = wingsiguazucontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = wingsiguazucontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield wingsiguazunotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield wingsiguazufunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                else if (resultadospin.result == constans.WIN) {
                    const ganhojson = wingsiguazucontrol_logic.default.taskWinJsonData(resultadospin.json)
                    const jsonData = wingsiguazucontrol_logic.default.GetWinJson(ganhojson, 0, ml, cs, bet, saldoatual);
                ...
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "wings-iguazu") {//大奖
                    const cartajson = wingsiguazucontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = wingsiguazucontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = wingsiguazucontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, wingsiguazucontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield wingsiguazufunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield wingsiguazufunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
