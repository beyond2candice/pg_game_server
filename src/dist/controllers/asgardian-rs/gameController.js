...
const asgardianrsfunctions_1 = __importDefault(require("../../functions/asgardian-rs/functions"));
...
const asgardianrsnotcash = __importDefault(require("../../jsons/asgardian-rs/notcash"));
const asgardianrscontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield asgardianrscontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield asgardianrsnotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * asgardianrscontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "asgardian-rs";
                const game_code = 1340277;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield asgardianrsnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield asgardianrsnotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield asgardianrscontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = asgardianrscontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = asgardianrscontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield asgardianrsnotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield asgardianrsfunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "asgardian-rs") {//大奖
                    const cartajson = asgardianrscontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = asgardianrscontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = asgardianrscontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, asgardianrscontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield asgardianrsfunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield asgardianrsfunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
