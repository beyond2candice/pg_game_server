...
const werewolfhuntfunctions_1 = __importDefault(require("../../functions/werewolf-hunt/functions"));
...
const werewolfhuntnotcash = __importDefault(require("../../jsons/werewolf-hunt/notcash"));
const werewolfhuntcontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield werewolfhuntcontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield werewolfhuntnotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * werewolfhuntcontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "werewolf-hunt";
                const game_code = 1615454;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield werewolfhuntnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield werewolfhuntnotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield werewolfhuntcontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = werewolfhuntcontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = werewolfhuntcontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield werewolfhuntnotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield werewolfhuntfunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                else if (resultadospin.result == constans.WIN) {
                    const ganhojson = werewolfhuntcontrol_logic.default.taskWinJsonData(resultadospin.json)
                    const jsonData = werewolfhuntcontrol_logic.default.GetWinJson(ganhojson, 0, ml, cs, bet, saldoatual);
                ...
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "werewolf-hunt") {//大奖
                    const cartajson = werewolfhuntcontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = werewolfhuntcontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = werewolfhuntcontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, werewolfhuntcontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield werewolfhuntfunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield werewolfhuntfunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
