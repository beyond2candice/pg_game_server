...
const heiststakesfunctions_1 = __importDefault(require("../../functions/heist-stakes/functions"));
...
const heiststakesnotcash = __importDefault(require("../../jsons/heist-stakes/notcash"));
const heiststakescontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield heiststakescontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield heiststakesnotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * heiststakescontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "heist-stakes";
                const game_code = 105;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield heiststakesnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                const resultadospin = yield heiststakescontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = heiststakescontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = heiststakescontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield heiststakesnotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                ...
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "heist-stakes") {//大奖
                    const cartajson = heiststakescontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = heiststakescontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, heiststakescontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
