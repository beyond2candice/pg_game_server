...
const maskcarnivalfunctions_1 = __importDefault(require("../../functions/mask-carnival/functions"));
...
const maskcarnivalnotcash = __importDefault(require("../../jsons/mask-carnival/notcash"));
const maskcarnivalcontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield maskcarnivalcontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield maskcarnivalnotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * maskcarnivalcontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "mask-carnival";
                const game_code = 118;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield maskcarnivalnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield maskcarnivalnotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield maskcarnivalcontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = maskcarnivalcontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = maskcarnivalcontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield maskcarnivalnotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield maskcarnivalfunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "mask-carnival") {//大奖
                    const cartajson = maskcarnivalcontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = maskcarnivalcontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = maskcarnivalcontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, maskcarnivalcontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield maskcarnivalfunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield maskcarnivalfunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
