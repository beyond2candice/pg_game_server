...
const legendofhouyifunctions_1 = __importDefault(require("../../functions/legend-of-hou-yi/functions"));
...
const legendofhouyinotcash = __importDefault(require("../../jsons/legend-of-hou-yi/notcash"));
const legendofhouyicontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield legendofhouyicontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield legendofhouyinotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * legendofhouyicontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "legend-of-hou-yi";
                const game_code = 34;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield legendofhouyinotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield legendofhouyinotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield legendofhouyicontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = legendofhouyicontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = legendofhouyicontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield legendofhouyinotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield legendofhouyifunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                else if (resultadospin.result == constans.WIN) {
                    const ganhojson = legendofhouyicontrol_logic.default.taskWinJsonData(resultadospin.json)
                    const jsonData = legendofhouyicontrol_logic.default.GetWinJson(ganhojson, 0, ml, cs, bet, saldoatual);
                ...
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "legend-of-hou-yi") {//大奖
                    const cartajson = legendofhouyicontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = legendofhouyicontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = legendofhouyicontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, legendofhouyicontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield legendofhouyifunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield legendofhouyifunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
