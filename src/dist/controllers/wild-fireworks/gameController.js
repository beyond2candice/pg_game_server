...
const wildfireworksfunctions_1 = __importDefault(require("../../functions/wild-fireworks/functions"));
...
const wildfireworksnotcash = __importDefault(require("../../jsons/wild-fireworks/notcash"));
const wildfireworkscontrol_logic = __importDefault(require("./controller_logic"));																	  
    ...
    getGameInfo(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const jsonData = yield wildfireworkscontrol_logic.default.getGameInfo(token);
                res.send(jsonData);
            }
            catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				if (!userRet || userRet.length < 1) {
		            res.send(yield wildfireworksnotcash.default.notcash(0, cs, ml));
                    return false;
				}
				const user = userRet[0];
                let bet = cs * ml * wildfireworkscontrol_logic.default.getMxl();
                console.log(bet);
                const gamename = "wild-fireworks";
                const game_code = 83;
                ...
                if (getBalanceRet.status !== 1) {
                    res.send(yield wildfireworksnotcash.default.notcash(user.saldo, cs, ml));
                    return false;
                }
                ...
                if (!(bet>0) && (saldoatual < bet)) {
                    const semsaldo = yield wildfireworksnotcash.default.notcash(saldoatual, cs, ml);
                    res.send(semsaldo);
                    return false;
                }
                const resultadospin = yield wildfireworkscontrol_logic.default.takeResultFromBet(user, agent, bet, saldoatual, token, gamename);
                if (resultadospin.result === constans.LOST) {//输
                    const perdajson = wildfireworkscontrol_logic.default.taskLostJsonData(resultadospin.json);
                    const jsonData = wildfireworkscontrol_logic.default.getLostJson(perdajson, ml, cs, bet, saldoatual)
                    ...
                    if (transRet.status !== 1) {
                        res.send(yield wildfireworksnotcash.default.notcash(saldoatual, cs, ml));
                        return false;
                    }
                    ...
                    yield wildfireworksfunctions_1.default.savejsonspin(user.id, JSON.stringify(json));
                 
                    res.send(json);
                } 
                
                
                else if (resultadospin.result === constans.BIGWIN && resultadospin.gamecode === "wild-fireworks") {//大奖
                    const cartajson = wildfireworkscontrol_logic.default.taskBonusJsonData(resultadospin.json)
                    ...
                    const jsonData = wildfireworkscontrol_logic.default.GetBonusJson(cartajson, currentStep, ml, cs, bet, saldoatual);
                    ...
                    if (currentStep === 0) {
                       ...
                       if (propertyCount == 1) {
                            ...
                            let historyJson = wildfireworkscontrol_logic.default.GetBonusJson(cartajson, propertyCount - 1, ml, cs, bet, saldoatual).json.dt.si;
                            let history = allfunctions_1.default.createGameHistory(game_code, historyJson);
                            for (var i= propertyCount - 2; i>0; i--) {
                                allfunctions_1.default.appendGameHistoryStep(history, wildfireworkscontrol_logic.default.GetBonusJson(cartajson, i, ml, cs, bet, saldoatual).json.dt.si);
                            }
                        ...
                        yield wildfireworksfunctions_1.default.completecallAndSaveJsonSpin(calltwo[0].id, user.id, JSON.stringify(json));
                        res.send(json);
                        return true;
                    }
                    yield wildfireworksfunctions_1.default.subtrairstepscallAndSaveJsonSpin(resultadospin.idcall, user.id, JSON.stringify(json));
                    res.send(json);
